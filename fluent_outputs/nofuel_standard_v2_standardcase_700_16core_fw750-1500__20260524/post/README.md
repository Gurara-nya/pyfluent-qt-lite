...
- Finest selected grid for this run: `fw1250`.
- The strict same-setup adjacent comparison `fw1000 -> fw1250` is within all configured thresholds.

## Campaign Completion Caveat

- Requested target grids were `fw750`, `fw1000`, `fw1250`, and `fw1500`; the `fw1500` 700-iteration target DAT was not produced.
- `fw1500` failed under the requested same setup: copied `standard.cas.h5`, combo-256, 700 target iterations, and 16 cores. Direct PyFluent attempts reached the early iteration phase and then lost the Fluent backend connection; Fluent journal recovery exposed the underlying numerical backend failure (`floating point exception` / node `SIGSEGV`).
- Therefore this package does **not** support a complete four-grid, `fw1500`-referenced grid-independence conclusion. The pass statement above applies only to the completed `fw750 -> fw1000 -> fw1250` subset, using `fw1250` as the finest available grid.
